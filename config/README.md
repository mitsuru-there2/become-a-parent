...
本編の`judgment_catalog: "routes-10"`はD-070の850判断を示します。5ステージ×17ルート×10判断で、各判断は単一ステージ・単一ルート・単一実行案です。数値と本文はJSONを直接編集します。新規人生はdata-14、保存済みのdata-13は固定済み設定で再生します。

## 現行の編集先（rules-14 / S-018）
...
保存には合成した設定とパック版を固定します。JSONの変更は新規ゲームから適用し、既存保存は旧設定で継続します。現在はrules-14 / data-15 / save-15です。設定JSONの家族状態効果は従来の10点単位で記載し、新版の実行時に割合へ換算します。旧保存の移行は行わず、互換性は保証しません。共通コンテンツ形式の`data_version`とパックの`requires_data`は互換形式名`data-2`を維持し、人生のルール・保存の版とは区別します。
...
| 現行ステージ制の家庭背景・収入・抽選・効果補正 | `decisions.json` の `difficulty_profiles` |
| 仕事の収入・時間・疲れ・充実                   | `work`                                    |
...
現行ステージ制の難易度は`difficulty_profiles`で調整します。開始年齢、家計、実家、イベント確率、有利・不利な効果、判断による収入に反映されます。詳細は[S-015](../docs/specs/content.md#現行ステージ制の難易度2026-09-22d-086)を参照してください。`base.json`の`difficulties`は従来方式の設定です。ゲームの計算方式、状態変数、成人後の進行・結末の判定式はTypeScript側です。新しい能力・効果の種類・時間軸・判定方式を作るときはSPECとエンジンも変更します。
