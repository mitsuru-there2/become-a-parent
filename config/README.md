...
旧rules-12では、`crossroads`の`required_decisions`に方針IDを並べていました。現行は上記のルートIDを使います。本編は0・4・8・12・16歳の春〜夏を岐路とし、各岐路で5分類すべてのルートを必須にします。未選択がある間、予測は進行不可となり、公開状態の`life.crossroad.missing`から画面・CLIが同じ不足項目を案内します。
