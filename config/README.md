...
保存には合成した設定とパック版を固定します。JSONの変更は新規ゲームから適用し、既存保存は旧設定で継続します。現在はrules-12 / data-12 / save-13です。共通コンテンツ形式の`data_version`とパックの`requires_data`は互換形式名`data-2`を維持し、人生のルール・保存の版とは区別します。
...
`crossroads`は年齢（月齢）で岐路を定義し、`required_decisions`にその期に明示選択が必要な方針IDを並べます。本編は0・4・8・12・16歳の春〜夏を岐路とし、初回は5分類すべての方針を必須にします。未選択がある間、予測は進行不可となり、公開状態の`life.crossroad.missing`から画面・CLIが同じ不足項目を案内します。
