...
保存には合成した設定とパック版を固定します。JSONの変更は新規ゲームから適用し、既存保存は旧設定で継続します。現在はrules-13 / data-14 / save-14です。旧保存の移行は行わず、互換性は保証しません。共通コンテンツ形式の`data_version`とパックの`requires_data`は互換形式名`data-2`を維持し、人生のルール・保存の版とは区別します。
