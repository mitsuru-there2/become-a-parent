...
`story-`で始まる本編の27判断・55選択肢・50イベントは、年代と選択をつなぐ物語（D-040）です。既存を含め判断56件・選択肢105件。追加経路は`bun run test:stories /tmp/任意の空フォルダ 2`で公開CLIから19経路×2シードを検証できます。履歴による思い出には`requires.history`、休止時に止める活動イベントには`requires.policies`を使います。ひとつの`once: true`行動で選んだ別の選択肢は再取得できないため、相互排他的な物語に使えます。分岐・頻度・保存互換の受け入れは[S-018 / AC-018-F〜H](../docs/specs/action-tree.md)、結果は[拡充検証](../docs/playtests/2026-09-18-stories.md)を参照してください。
