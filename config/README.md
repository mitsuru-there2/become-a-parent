...
保存には合成した設定とパック版を固定します。JSONの変更は新規ゲームから適用し、既存保存は旧設定で継続します。現在はrules-11 / data-11 / save-12です。共通コンテンツ形式の`data_version`とパックの`requires_data`は互換形式名`data-2`を維持し、人生のルール・保存の版とは区別します。
...
rules-10では`life_game.route_groups`で長期ルート群と転換準備の判断IDを定義します。学校の各所属方針には`route_group: "school"`、園から高校までの`route_stage: 0〜3`を指定し、各選択肢の`route`を4系統のIDへ対応させます。各段階のルート専用行動と18歳以降の行動には、判断側の`route_group`・`route_stage`・`route`と所属先の`requires.policies`または高校の`requires.history`を指定します。転換準備の単発行動は各選択肢の`switch_to`で移行先を宣言します。準備実行の次期だけ移行でき、同系統の進級には開始費を重ねません。未知のルート、段階の欠落、同一段階の重複は設定検査で拒否します。判定と予測は[S-018 / D-063](../docs/specs/action-tree.md#大ルートと学校の4系統2026-09-21d-063)に従い共通エンジンで行います。

rules-11では家庭生活・実家・遊び・放課後・仕事・家計にも同じ`route_groups`を追加します。これらの群は`layout: "branches"`と5段階の`stage_labels`を持ち、0〜19歳を覆う1件の大方針の選択肢に`route`を指定します。専用の行動・継続方針には判断側の`route_group`・`route_stage`・`route`を指定し、所属ルートが違うと選べません。`route`を省いた判断は共通行動の列に表示され、別ルート中でも使えます。`route_stage`は開始年齢を0〜3歳=0、4〜7歳=1、8〜11歳=2、12〜15歳=3、16〜19歳=4へ対応させます。転換準備は各群の`switch_decision`で指定し、前期に実行した次期だけ新ルートを選べます。現行の本編は判断91件・選択肢169件です。正本は[S-018 / D-065](../docs/specs/action-tree.md#5分類の縦型大方針2026-09-21d-065)。

教育・進路では任意の`tree_route`を判断または選択肢に指定し、取得条件を持たないアクションも4本の表示レーン内へ配置します。`tree_route`は画面配置だけに使い、所属制限には使いません。取得可能性を制限する場合は従来どおり`route`と前提条件を指定します。教育の`stage_labels`は園準備から18歳以降までの5段階の見出しです。正本は[S-018 / D-066](../docs/specs/action-tree.md#教育ツリーの一方向配置2026-09-21d-066)。
