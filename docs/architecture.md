...
2026-09-14。Godotをエンジンとするテキスト版。技術選定の理由は[D-017](decisions.md)、操作と保存の正本は[S-008・S-011](specs/cli.md)。
...
ゲームの計算はすべてGDScript。Pythonの方針プリセットは入力補助で、ゲームの計算を持たない。描画や外部サービスは使わない。Godotの導入後、プレイにネットワーク接続は不要。
...
`project.godot` をGodotで開ける。現段階はシーン・ビジュアルを作らず、起動はCLIを使う。画面を追加するときも同じGDScriptの公開状態と確定処理を呼ぶ。CLIは比較・再現用として残す。イベントの追加時は安定ID、仕様、データ版と影響するテストを同時に更新する。
